...
const DataTableComponent = () => {
  ...
  useEffect(() => {
    const fetchBlockHistory = async () => {
      ...
      try {
        ...
        const response = await axios.get(`${API_URL}/admin/getBlockHistory`, {
  ...
  const filteredData = isSearch
    ? blockHistory.filter((user) =>
        String(user?.user?.name || user?.user?.email || user?.user?.phoneNumber || "")
          .toLowerCase()
  ...
  return (
    <Fragment>
      ...
      <DataTable
        ...
        noDataComponent={<div className="text-center p-4">No block history found</div>}
      />
