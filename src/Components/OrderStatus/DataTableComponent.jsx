...
import { fetchOrderStatuses, updateOrderStatus } from "../../Redux/stateSlice/orderStatusReducer";
import { getProductImageFallbackLabel, getProductImageSource } from "../../Utils/productImage";

const ORDER_STATUS_OPTIONS = ["Processing", "Confirmed", "Shipped", "Delivered", "Cancelled"];
...
const DataTableComponent = () => {
  ...
  useEffect(() => {
    setStatusDrafts((previousDrafts) => {
      ...
      (orders || []).forEach((order) => {
        if (order?._id && !Object.prototype.hasOwnProperty.call(nextDrafts, order._id)) {
          nextDrafts[order._id] = order?.status || "Processing";
        }
      });
  ...
  const handleStatusUpdate = async (row) => {
    ...
    const nextStatus = statusDrafts[row._id] || row?.status || "Processing";
      ...
      },
      {
        ...
        cell: (row) => (
          <div className="d-flex align-items-center gap-2 py-1" style={{ minWidth: "230px" }}>
            <Input
              type="select"
              value={statusDrafts[row?._id] || row?.status || "Processing"}
            ...
            <Btn
              attrBtn={{
                color: "primary",
                size: "sm",
                disabled: submitting || (statusDrafts[row?._id] || row?.status) === row?.status,
