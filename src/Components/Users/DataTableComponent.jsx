...
const getUserDisplayName = (row) =>
  ...
  row?.email ||
  row?.phoneNumber ||
  (row?._id ? `User ${String(row._id).slice(-4)}` : "Unknown User");
...
const AvatarCell = ({ row }) => {
  ...
  if (!avatarUrl || hasImageError) {
    return (
      ...
      >
        {getInitials(getUserDisplayName(row))}
      </div>
    );
  }
...
const reasonPlaceholder = currentToggleUser
  ...
  const handleBlockSubmit = async () => {
    ...
    try {
      ...
      await axios.put(
        `https://api.careavatar.com/api/admin/blockUser/${userId}`,
    ...
    },
    {
      ...
      cell: (row) => (
        <div className="text-start">
          <div className="fw-semibold">{getUserDisplayName(row)}</div>
