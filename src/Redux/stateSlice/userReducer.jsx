...
import { API_URL, BASE_URL, LOCAL_URL } from "../../Config/AppConstant";
...
export const searchUsers = (query) => async (dispatch) => {
  ...
  try {
    ...
    const data = await axios.get(
      `${API_URL}/admin/search?query=${query}`,
      {
        headers: {
  ...
  (page = 1, limit = 10) =>
    async (dispatch) => {
      ...
      try {
        ...
        const { data } = await axios.get(
          `${API_URL}/admin/getAllUsers?page=${page}&limit=${limit}`,
          {
            headers: {
...
export const editUser = (id, updatedData) => async (dispatch) => {
  ...
  try {
    const token = localStorage.getItem("token");
    const { data } = await axios.put(`${API_URL}/admin/editUser/${id}`, updatedData, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
...
export const changUserStatus = (id, status) => async (dispatch) => {
  ...
  try {
    const token = localStorage.getItem("token");
    const { data } = await axios.put(`${BASE_URL}/api/admin/updateUserStatus/${id}`, { status }, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
