...
export const addStory = (newStory) => async (dispatch) => {
  ...
  try {
    ...
    dispatch({
      ...
      payload: error.response?.data?.message || "Failed to add story",
    });
  }
};
...
export const editStory = (id, updatedData) => async (dispatch) => {
  ...
  try {
    ...
    dispatch({
      ...
      payload: error.response?.data?.message || "Failed to edit story",
    });
  }
};
...
const storyReducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case EDIT_STORY_SUCCESS:
      return { ...state, editLoading: false, error: null };
